...
int	hit(const t_rayon *r, double t_max, t_hit_record *rec, t_obj *obj)
{
	...
	if (hit_anything && (rec->hit_obj->bump_map /*|| (s->hit_obj && rec->hit_obj->n == s->hit_obj->n)*/))
	{
		...
		col = rec->hit_obj->bump_map_addr[(super_mod((double)rec->p.x * 100.0, rec->hit_obj->bump_height))
				* rec->hit_obj->bump_width + (super_mod((double)rec->p.z * 100.0, rec->hit_obj->bump_width))];
		// printf("col : %x\n", col);
